...
public class Solution {
    // tc O(4^(n-m)), sc O(n-m), where n - number of cells in a room and m - number of obstacles
    // 4 ms, faster than 90.41%; 39.5 MB, less than 81.96%
    private Robot robot;
    ...
    private void backtrack(int i, int j, int d, Set<Pair<Integer, Integer>> visited) {
        ...
        for (int k = 0; k < 4; k++) {
            ...
            int newJ = i + dirs[newD][1];
            if (!visited.contains(new Pair<>(newI, newJ)) && robot.move()) {
                backtrack(newI, newJ, newD, visited);
                goBack();
            }
            robot.turnRight();
        }
