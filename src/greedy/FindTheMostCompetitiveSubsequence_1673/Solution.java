 ...
 * [1,3,4] is more competitive than [1,3,5] because the first position they differ is at the final number, and 4 is less than 5.
 *
 * Example 1:
...
public class Solution {
    public int[] mostCompetitive(int[] nums, int k) {
        int n = nums.length, attempts = n - k;
        Deque<Integer> stack = new ArrayDeque<>();
        for (int num : nums) {
            while (attempts > 0 && !stack.isEmpty() && num < stack.peekLast()) {
                stack.pollLast();
                attempts--;
            }
            stack.addLast(num);
