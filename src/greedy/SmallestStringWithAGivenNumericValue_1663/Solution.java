package greedy.SmallestStringWithAGivenNumericValue_1663;

/**
 * 1663. Smallest String With A Given Numeric Value
 * https://leetcode.com/problems/smallest-string-with-a-given-numeric-value/
 *
 * The numeric value of a lowercase character is defined as its position (1-indexed) in the alphabet,
 * so the numeric value of a is 1, the numeric value of b is 2, the numeric value of c is 3, and so on.
 *
 * The numeric value of a string consisting of lowercase characters is defined as the sum of its
 * characters' numeric values. For example, the numeric value of the string "abe" is equal to 1 + 2 + 5 = 8.
 *
 * You are given two integers n and k. Return the lexicographically smallest string with length equal
 * to n and numeric value equal to k.
 *
 * Note that a string x is lexicographically smaller than string y if x comes before y in dictionary order,
 * that is, either x is a prefix of y, or if i is the first position such that x[i] != y[i], then x[i]
 * comes before y[i] in alphabetic order.
 *
 * Example 1:
 * Input: n = 3, k = 27
 * Output: "aay"
 * Explanation: The numeric value of the string is 1 + 1 + 25 = 27, and it is the smallest string with such a value and length equal to 3.
 *
 * Example 2:
 * Input: n = 5, k = 73
 * Output: "aaszz"
 *
 * Constraints:
 * 1 <= n <= 10^5
 * n <= k <= 26 * n
 */
public class Solution {
    // tc O(n); sc O(n)
    // 16 ms, faster than 52.95%; 38.6 MB, less than 98.78%
    // If you build the string from the end to the beginning, it will always be optimal to put the highest possible character at the current index
    public String getSmallestString(int n, int k) {
        char[] res = new char[n];
        for (int remains = n - 1; remains >= 0; remains--) {
            int curChar = Math.min(k - remains, 26);
            res[remains] = (char) ('a' + curChar - 1);
            k -= curChar;
        }
        return new String(res);
    }

    // TLE, brute force
    // tc O(26^n)?, sc O(nk)
    public String getSmallestString1(int n, int k) {
        String[][] memo = new String[n + 1][k + 1];
        return helper(n, k, new StringBuilder(), memo);
    }

    private String helper(int n, int k, StringBuilder sb, String[][] memo) {
        if (memo[n][k] != null) return memo[n][k];
        if (n == 0) {
            return k == 0 ? sb.toString() : null;
        }
        for (int i = 1; i <= 26; i++) {
            if (n > 1 && k - i > 0 || n == 1 && k - i == 0) {
                char c = (char) ('a' + (i - 1));
                int len = sb.length();
                sb.append(c);
                String s = helper(n - 1, k - i, sb, memo);
                if (s != null) {
                    memo[n][k] = s;
                    return s;
                }
                sb.setLength(len);
            }
        }
        return null;
    }
}
