package dynamic.UncrossedLines_1035;
...
public class Solution {
    ...
    public int maxUncrossedLines(int[] A, int[] B) {
        ...
        return bt(A, B, 0, 0, dp);
    }

    private int bt(int[] A, int[] B, int i, int j, int[][] dp) {
        if (i == A.length || j == B.length) return 0;
        if (dp[i][j] != -1) return dp[i][j];
        if (A[i] == B[j]) {
            dp[i][j] = 1 + bt(A, B, i + 1, j + 1, dp);
        } else {
            dp[i][j] = Math.max(bt(A, B, i, j + 1, dp), bt(A, B, i + 1, j, dp));
        }
        return dp[i][j];
    }
}
