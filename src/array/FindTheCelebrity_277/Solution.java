 ...
 * is in the party. Return the celebrity's label if there is a celebrity in the party. If
...
public class Solution {
    // tc O(n), sc O(1)
    // 18 ms, faster than 93.57%; 39.5 MB, less than 100.00%
    public int findCelebrity(int n) {
        ...
        return checkCelebrity(celebrityCandidate, n) ? celebrityCandidate : -1;
    }

    private boolean checkCelebrity(int celebrityCandidate, int n) {
        for (int i = 0; i < n; i++) {
    ...
    public int findCelebrity1(int n) {
        for (int i = 0; i < n; i++) {
            boolean i_knows_at_least_one_j = false;
            boolean all_j_knows_i = true;
            for (int j = 0; j < n; j++) {
                if (i == j) continue;
                i_knows_at_least_one_j |= knows(i, j);
                if (i_knows_at_least_one_j) break;
                all_j_knows_i &= knows(j, i);
                if (!all_j_knows_i) break;
            }
            if (!i_knows_at_least_one_j && all_j_knows_i) return i;
        }
        return -1;
    }
