 ...
 *
 * Return an array of the k-values of the pancake flips that should be performed in order to
 * sort A. Any valid answer that sorts the array within 10 * A.length flips will be judged as correct.
 *
 * Example 1:
 * Input: A = [3,2,4,1]
 * Output: [4,2,4,3]
...
public class Solution {
    ...
    private void flip(int[] A, int tillInd) {
        for (int i = 0; i < tillInd / 2; i++) {
            int tmp = A[i];
            A[i] = A[tillInd - i - 1];
            A[tillInd - i - 1] = tmp;
        }
